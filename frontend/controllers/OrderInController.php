...
class OrderInController extends Controller
{
    ...
    public function actionAttachments() {
        ...
        if (isset($_FILES['attachment']) && isset($_GET['orderInId'])) {

            $fileStoragePath = \common\models\Settings::getFileStoragePath();

            $fileDestination = $fileStoragePath.'/'.$_GET['orderInId'];
        ...
        $attachments = [];
        if (isset($_GET['orderInId'])) { 
            $orderIn = \common\models\OrderIn::findOne($_GET['orderInId']);
            $attachments = $orderIn->getAttachments();
        }
